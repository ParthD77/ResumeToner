...
export function extractJsonFromChat(value: string) {
  ...
  // ChatGPT web research can inject internal citation markers into JSON
  // strings. Some marker variants contain unescaped double quotes (for
  // example, {index="0"}), making otherwise valid JSON impossible to parse.
  // They are UI metadata rather than user content, so remove them before
  // parsing. Real source URLs in the response remain untouched.
  const json = unfenced
    .slice(start, end + 1)
    .replace(
      /:(?:chatgpt-content-reference|contentReference)(?:\[[^\]\r\n]*\])?\{[^{}\r\n]*\}/g,
      "",
    )
    .replace(/cite[^\r\n]*/g, "");
