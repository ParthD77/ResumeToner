...
describe("local desktop sessions", () => {
  ...
  it("defaults recovery reviews when loading an older session", () => {
    const session = emptySession("original");
    const { recoveryReviews: _removed, ...olderSession } = session;
    const restored = readArchive({
      getItem: (key) => key === SESSION_KEY
        ? JSON.stringify({ version: 1, session: olderSession, history: [] })
        : null,
    });
    expect(restored.session.recoveryReviews).toEqual({});
  });
